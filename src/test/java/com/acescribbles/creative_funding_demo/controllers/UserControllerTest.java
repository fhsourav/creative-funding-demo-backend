...
public class UserControllerTest {
	...
	@Test
	public void testThatCreateSubscriberSuccessfullyReturnsStatusCreatedAndSavedUser() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.post("/api/v1/users/register/subscriber")
				...
				MockMvcResultMatchers.jsonPath("$.displayName").value("Test User")
		).andExpect(
				MockMvcResultMatchers.jsonPath("$.handle").value("testUser")
		).andExpect(
				MockMvcResultMatchers.jsonPath("$.createdAt").exists()
		).andExpect(
				MockMvcResultMatchers.jsonPath("$.lastModifiedAt").doesNotExist()
		);
